...
module.exports = {
	...
	async execute(interaction, client) {
		...
		if (subCommand == 'start') {
				...
				collector.on('collect', m => {
					...
					collector2.on('collect', m => {
						...
						if (color == 'black') {
							...
							// fs.writeFileSync('./lib/database/games/chess/chess_games.json', JSON.stringify(games, null, 2))
							boardNew = functions.drawBoard(JSON.parse(fs.readFileSync('./lib/database/games/chess/chess_games.json', 'utf8'))[interaction.guild.id].games[gameID].boardConfig)
			...
			fs.writeFileSync('./lib/database/games/chess/chess_games.json', JSON.stringify(games, null, 2))
			boardNew = functions.drawBoard(currentGame.boardConfig)
			var attachment = new AttachmentBuilder(boardNew[0].toBuffer('image/png'));
			...
			interaction.followUp({ content: `You've moved from ${from.toUpperCase()} to ${to.toUpperCase()}! ${(currentGame.botPlaying) ? 'Now it\'s my turn!' : `It's now <@${currentGame[currentGame.boardConfig.turn]}>'s turn!`}`, files: [attachment] })
			if (currentGame.botPlaying) {
				...
				return interaction.editReply({ content: `I'll move from ${Object.keys(AIMoved)} to ${Object.values(AIMoved)}! It's your turn now! (${prefix}chess move <from> <to>)`, files: [attachment] })
			}
		}
